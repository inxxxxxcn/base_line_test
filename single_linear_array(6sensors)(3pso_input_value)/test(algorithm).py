...
if __name__ == "__main__":
    ...
    ba = ba.subs(I, 1.13).subs(D, -0.18).subs(m, 152)
    sensor_number = 6
    l0, l1, l2, l3, l4 = sym.symbols("l0, l1, l2, l3, l4")
    xi = [-l0 - l1 - l2 - l3 - l4, -l1 - l2 - l3 - l4, -l2 - l3 - l4, -l3 - l4, -l4, 0]
    ...
    ggdbl4 = sym.diff(gdbx[4] + gdby[4] + gdbz[4], l4)
    size = 50
    li = np.linspace(0, 100, size)
    Gpx = np.zeros(shape=size)
    Gpy = np.zeros(shape=size)
    Gpz = np.zeros(shape=size)
    for i in range(0, size):
        Gpx[i] = ggdbl4.subs(l4, li[i]).subs(x1, 50).subs(y1, -50).subs(z1, 10)
    plt.plot(li, Gpz)
    plt.show()
